...
export const applybytitleMock: CandidateCard[] = sectionConfigs.flatMap(
  (config, sectionIndex) =>
    Array.from({ length: config.count }, (_, index) => {
      ...
      return {
        id,
        name: candidateNames[
          (index + sectionIndex * 2) % candidateNames.length
        ],
        status: config.status,
        section: config.section,
        appliedAt:
          appliedDates[(index + sectionIndex * 3) % appliedDates.length],
        badgeText:
          badgeTextLabels[(index + sectionIndex * 4) % badgeTextLabels.length],
        skillMatch:
          skillMatchLabels[(index + sectionIndex) % skillMatchLabels.length],
        viewed:
          config.section === "newApplied" ? index % 4 === 0 : index % 3 !== 0,
